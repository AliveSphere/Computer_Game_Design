...
import onelife.obstacles.testObstacles;
...
public class testLevel extends Level {
		...
		testObstacles o = new testObstacles();
...
//			layer1.addChild(ground, layer1);
			...
			for (Sprite platform: o.getPlatforms()) {
				layer1.addChild(platform, layer1);
			}
			for (Sprite lava: o.getLava()) {
				layer2.addChild(lava, layer2);
			}
			for (Sprite block: o.getBlocks()) {
				layer3.addChild(block, layer3);
			}
			for (Sprite spike: o.getSpikes()) {
				layer4.addChild(spike, layer4);
			}
			...
			/*ParticleEmitter pe1 = new ParticleEmitter("emitter 1", layer1);
			pe1.addEmitter(800, 120);
			layer1.addChild(pe1, layer1);*/
			...
			if (door != null) {
				if (reachedEnd()) {
					if (doorOpen == false) {
						...
						pr.totalScore += this.coinCounter;
					}
					doorOpen = true;
				}
			}
...
//						System.out.println(players.get(3).isJumping());
					...
					if (player.landsOn(ground) && !(players.get(0) == player)) {
						player.setAcceleration(0);
						player.setVelocity(0);
						player.setJumping(false);
					}
...
//		      					if (!p.isJumping())
	...
	//	                   	inBounds(players.get(x), players.get(x).getXpos(), (int)(players.get(x).getYpos() + players.get(x).getVelocity()));
				...
				if (inAbyss && players.get(0).isAlive()) {
	           		 int layerX = layers.get(0).xpos;
	           		 int layerY = layers.get(0).ypos;
	           		 players.get(0).setAlive(false);
	           		 Sprite layerDead = new Sprite("LayerDead", "LayerDead.png");
	       			 levelOne.removeChild(layers.get(0));
...
//				pl.setAlive(false);
		...
		public boolean collidesBlock(Player p, testObstacles o) {
	    	for(Sprite block : o.getBlocks()) {
		...
		public boolean collidesPlatform(Player p, testObstacles o) {
	    	for(Sprite platform : o.getPlatforms()) {
