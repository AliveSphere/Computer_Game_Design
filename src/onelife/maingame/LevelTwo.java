...
import onelife.obstacles.LevelTwoObstacles;
...
public class LevelTwo extends Level {
	...
	LevelTwoObstacles o = new LevelTwoObstacles();
...
//		Sprite platform1 = new Sprite("Platform 1", "ledge.png");
		...
		for (Sprite spike: o.getSpikes()) {
			layer1.addChild(spike, layer1);
		}
		for (Sprite platform: o.getPlatforms()) {
			layer2.addChild(platform, layer2);
		}
		for (Sprite lava: o.getLava()) {
			layer3.addChild(lava, layer3);
		}
		for (Sprite block: o.getBlocks()) {
			layer4.addChild(block, layer4);
		}
...
//					System.out.println(players.get(3).isJumping());
				...
				if (player.landsOn(ground) && !(players.get(1) == player)) {
					//System.out.println(players.get(2).isJumping());
					player.setAcceleration(0);
					player.setVelocity(0);
					player.setJumping(false);
...
//	      						 p.animate("running");
	//                   	inBounds(players.get(x), players.get(x).getXpos(), (int)(players.get(x).getYpos() + players.get(x).getVelocity()));
			...
			if (inAbyss && players.get(1).isAlive()) {
          		 int layerX = layers.get(1).xpos;
          		 int layerY = layers.get(1).ypos;
          		 players.get(1).setAlive(false);
          		 Sprite layerDead = new Sprite("LayerDead", "LayerDead.png");
      			 levelOne.removeChild(layers.get(1));
...
//			pl.setAlive(false);
	...
	public boolean collidesBlock(Player p, LevelTwoObstacles o) {
    	for(Sprite block : o.getBlocks()) {
	...
	public boolean collidesPlatform(Player p, LevelTwoObstacles o) {
    	for(Sprite platform : o.getPlatforms()) {
